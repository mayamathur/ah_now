...
function(input, output, session) {
  ...
  detail <- reactive({
      # subset to chosen platforms
     d <- get_data( metric = input$metric3,
                start.date = format( input$dateRange3[1] ),
                end.date = format( input$dateRange3[2] ),
                .platforms = input$plotPlatforms3,
                helpers = TRUE
                )
     d<-HelperDetailSummarise(.metric = input$metric3, .data = d)
     return(d)
  })
  ...
  ######################show each Helper Detail Displayed############
  #Testing
  # output$txt <- renderText({
  #   e = detail()
  #   paste(class(e$date))
  # })
  output$helperDetail_Displayed_Map = renderPlotly({
    ...
    f$string = paste( f$helpers, " (", input$metric3," = ", f$total,")", sep="" )
    ...
    p = ggplot(data = states) + 
      ...
      geom_point( data = f, mapping=aes( x = Longitude, y = Latitude, size = total, text=string), color = "red", alpha = 1/2, size = 1 ) +
      coord_fixed(1.3) +
      guides(fill=FALSE) +
      theme_classic() +
      xlab("Longitude") +
  ...
  output$HelperDetail_Displayed <- DT::renderDataTable({
    e <- detail()
    e <- select(e,c("helpers", "region", "total"))
    e <- e[order(-e$total),] 
    DT::datatable(data = e,
  ...
  output$helperMap = renderPlotly({
    ...
    # for hovering over points
    f$string = paste( f$helpers, " (", f$HelperType_Pub,")", sep="" )
